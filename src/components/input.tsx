...
export const Input: FC<Props> = ({ label, value, placeholder, onChange, tail, type = 'text' }) => {
  const id = useId();
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <div className="relative">
        ...
        />
        {tail && <span className="absolute h-full items-center flex px-4 text-slate-600 bottom-0 right-0">{tail}</span>}
      </div>
    </div>
  );
};
